...
#用户加入多个俱乐部（目前接口调用不能很好支持此case，所以由手工测试覆盖）
class T103_UserJoinClub_02NormalTest(ApiTestSuit):
    """API接口测试类T103_UserJoinClub_02NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T103_UserJoinClub_02NormalTest.__name__
        self.testure_path = r"testure/T103_UserJoinClub_02NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT103_UserJoinClub_02Normal)
        ################################################################################

    def testT103_UserJoinClub_02Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T103_UserJoinClub_02NormalTest, self).beforeHttpTest(context=context)
