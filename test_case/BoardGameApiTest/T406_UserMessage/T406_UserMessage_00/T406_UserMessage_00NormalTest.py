...
class T406_UserMessage_00NormalTest(ApiTestSuit):
    """API接口测试类T406_UserMessage_00NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T406_UserMessage_00NormalTest.__name__
        self.testure_path = r"testure/T406_UserMessage_00NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT406_UserMessage_00Normal)
        ################################################################################

    def testT406_UserMessage_00Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T406_UserMessage_00NormalTest, self).beforeHttpTest(context=context)
