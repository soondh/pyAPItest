...
#返回结果为空(没有符合条件的俱乐部)
class T104_ClubFuzzySearch_00NormalTest(ApiTestSuit):
    """API接口测试类T104_ClubFuzzySearch_00NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T104_ClubFuzzySearch_00NormalTest.__name__
        self.testure_path = r"testure/T104_ClubFuzzySearch_00NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT104_ClubFuzzySearch_00Normal)
        ################################################################################

    def testT104_ClubFuzzySearch_00Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T104_ClubFuzzySearch_00NormalTest, self).beforeHttpTest(context=context)
