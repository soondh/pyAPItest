...
#选择坐标，未选游戏
class T104_ClubFuzzySearch_02NormalTest(ApiTestSuit):
    """API接口测试类T104_ClubFuzzySearch_02NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T104_ClubFuzzySearch_02NormalTest.__name__
        self.testure_path = r"testure/T104_ClubFuzzySearch_02NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT104_ClubFuzzySearch_02Normal)
        ################################################################################
    ...
    def env_setup(self):
        ...
        # 修改csv
        headers = ['caseId', 'caseDesc', 'isRun', 'priority', 'httpType', 'apiPath', 'verifyReCode', 'paramUrl.game_id', 'paramUrl.province',
                   'paramUrl.city', 'paramUrl.district', 'paramUrl.page']
        rows = [('01', '', 'true', 1, 'post', '/club/fuzzySearch?', 200, '', '浙江省', '杭州市', '西湖区', 1)]

        curr_path = os.path.dirname(__file__)
        with open(curr_path + r'\testure\T104_ClubFuzzySearch_02NormalTest.csv', 'w', encoding='utf-8') as f:
            f_csv = csv.writer(f)
    ...
    def testT104_ClubFuzzySearch_02Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T104_ClubFuzzySearch_02NormalTest, self).beforeHttpTest(context=context)
