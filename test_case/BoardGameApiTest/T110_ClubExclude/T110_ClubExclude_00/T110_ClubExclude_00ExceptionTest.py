...
class T110_ClubExclude_00ExceptionTest(ApiTestSuit):
    """API接口测试类T110_ClubExclude_00ExceptionTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T110_ClubExclude_00ExceptionTest.__name__
        self.testure_path = r"testure/T110_ClubExclude_00ExceptionTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT110_ClubExclude_00Exception)
        ################################################################################

    def testT110_ClubExclude_00Exception(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T110_ClubExclude_00ExceptionTest, self).beforeHttpTest(context=context)
