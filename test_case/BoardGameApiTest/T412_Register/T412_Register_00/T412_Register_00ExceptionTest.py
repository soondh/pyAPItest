...
class T412_Register_00ExceptionTest(ApiTestSuit):
    """API接口测试类T412_Register_00ExceptionTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T412_Register_00ExceptionTest.__name__
        self.testure_path = r"testure/T412_Register_00ExceptionTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT412_Register_00Exception)
        ################################################################################

    def testT412_Register_00Exception(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T412_Register_00ExceptionTest, self).beforeHttpTest(context=context)
