...
#空密码
class T412_Register_01NormalTest(ApiTestSuit):
    """API接口测试类T412_Register_01NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T412_Register_01NormalTest.__name__
        self.testure_path = r"testure/T412_Register_01NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.env_setup()
        self.initConfig()
        self.regist_func(self.testT412_Register_01Normal)
        ################################################################################
    ...
    def env_setup(self):
        ...
        # 修改csv
        headers = ['caseId', 'caseDesc', 'isRun', 'priority', 'httpType', 'apiPath', 'verifyReCode',
                   'paramUrl.username',
                   'paramUrl.password']
        rows = [('01', '', 'true', 1, 'post', '/register?', 200, user_name, '')
                ]

        curr_path = os.getcwd()
        with open(curr_path + r'\testure\T412_Register_01NormalTest.csv', 'w', encoding='utf-8') as f:
            f_csv = csv.writer(f)
