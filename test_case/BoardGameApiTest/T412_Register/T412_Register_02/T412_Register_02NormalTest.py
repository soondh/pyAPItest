...
#根据当前时间确认用户名
class T412_Register_02NormalTest(ApiTestSuit):
    """API接口测试类T412_Register_02NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T412_Register_02NormalTest.__name__
        self.testure_path = r"testure/T412_Register_02NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.env_setup()
        self.initConfig()
        self.regist_func(self.testT412_Register_02Normal)
        ################################################################################
    ...
    def env_setup(self):
        ...
        # 修改csv
        headers = ['caseId', 'caseDesc', 'isRun', 'priority', 'httpType', 'apiPath', 'verifyReCode', 'cd','paramUrl.username',
                   'paramUrl.password']
        rows = [('01', '', 'true', 1, 'post', '/register?', 200, 0, user_name, 123)]

        curr_path = os.path.dirname(__file__)
        with open(curr_path + r'\testure\T412_Register_02NormalTest.csv', 'w', encoding='utf-8') as f:
            f_csv = csv.writer(f)
