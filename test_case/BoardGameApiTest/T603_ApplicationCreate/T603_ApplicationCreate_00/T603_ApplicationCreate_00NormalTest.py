...
#成功创建加入申请
class T603_ApplicationCreate_00NormalTest(ApiTestSuit):
    """API接口测试类T603_ApplicationCreate_00NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T603_ApplicationCreate_00NormalTest.__name__
        self.testure_path = r"testure/T603_ApplicationCreate_00NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.env_setup()
        self.initConfig()
        self.regist_func(self.testT603_ApplicationCreate_00Normal)
        ################################################################################

    def env_setup(self):
        """
        调用T412_00创建新用户 user2
        调用T101_00创建俱乐部 user1 club1
        :return:
        """
        #创建新用户（发送申请）
        ...
        cur.execute("SELECT id FROM zhuoyou_club where user_id=%s",user1_id)
        club_id = cur.fetchone()[0]
        conn.close()

        # 修改csv
        headers = ['caseId', 'caseDesc', 'isRun', 'priority', 'httpType', 'apiPath', 'verifyReCode', 'paramUrl.user_id',
                   'paramUrl.club_id', 'paramUrl.content']
        rows = [('01', '', 'true', 1, 'post', '/application/create?', 200, user2_id, club_id,'test')]

        curr_path = os.path.dirname(__file__)
        with open(curr_path + r'\testure\T603_ApplicationCreate_00NormalTest.csv', 'w', encoding='utf-8') as f:
            f_csv = csv.writer(f)
    ...
    def testT603_ApplicationCreate_00Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T603_ApplicationCreate_00NormalTest, self).beforeHttpTest(context=context)
