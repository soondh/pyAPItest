...
#cd:4 msg:没有权限
class T602_ApplicationCheck_02NormalTest(ApiTestSuit):
    """API接口测试类T602_ApplicationCheck_02NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T602_ApplicationCheck_02NormalTest.__name__
        self.testure_path = r"testure/T602_ApplicationCheck_02NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.env_setup()
        self.initConfig()
        self.regist_func(self.testT602_ApplicationCheck_02Normal)
        ################################################################################

    def env_setup(self):
        """
        调用T603_00创建申请,调用T412_02创建无权限用户
        :return:
        """
        ...
        with open(curr_path + r'\testure\T602_ApplicationCheck_02NormalTest.csv', 'w', encoding='utf-8') as f:
            f_csv = csv.writer(f)
    ...
    def testT602_ApplicationCheck_02Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T602_ApplicationCheck_02NormalTest, self).beforeHttpTest(context=context)
