...
#cd:2  msg:审核已经完成
class T602_ApplicationCheck_01NormalTest(ApiTestSuit):
    """API接口测试类T602_ApplicationCheck_01NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T602_ApplicationCheck_01NormalTest.__name__
        self.testure_path = r"testure/T602_ApplicationCheck_01NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.env_setup()
        self.initConfig()
        self.regist_func(self.testT602_ApplicationCheck_01Normal)
        ################################################################################
    ...
    def env_setup(self):
        ...
        test_script.run_test_class(case_name=["T602_ApplicationCreate_00NormalTest"])
        ...
        with open(curr_path + r'\testure\T602_ApplicationCheck_01NormalTest.csv', 'w', encoding='utf-8') as f:
            f_csv = csv.writer(f)
    ...
    def testT602_ApplicationCheck_01Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T602_ApplicationCheck_01NormalTest, self).beforeHttpTest(context=context)
