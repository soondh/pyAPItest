...
class T403_UserAttentionUser_00NormalTest(ApiTestSuit):
    """API接口测试类T403_UserAttentionUser_00NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T403_UserAttentionUser_00NormalTest.__name__
        self.testure_path = r"testure/T403_UserAttentionUser_00NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT403_UserAttentionUser_00Normal)
        ################################################################################

    def testT403_UserAttentionUser_00Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T403_UserAttentionUser_00NormalTest, self).beforeHttpTest(context=context)
