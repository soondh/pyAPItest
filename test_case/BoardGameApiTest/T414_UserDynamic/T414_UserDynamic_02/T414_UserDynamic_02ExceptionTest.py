...
class T414_UserDynamic_02ExceptionTest(ApiTestSuit):
    """API接口测试类T414_UserDynamic_02ExceptionTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T414_UserDynamic_02ExceptionTest.__name__
        self.testure_path = r"testure/T414_UserDynamic_02ExceptionTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT414_UserDynamic_02Exception)
        ################################################################################

    def testT414_UserDynamic_02Exception(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T414_UserDynamic_02ExceptionTest, self).beforeHttpTest(context=context)
