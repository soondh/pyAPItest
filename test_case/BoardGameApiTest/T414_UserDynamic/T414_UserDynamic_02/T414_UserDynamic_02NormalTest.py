...
#一条图片朋友圈
class T414_UserDynamic_02NormalTest(ApiTestSuit):
    """API接口测试类T414_UserDynamic_02NormalTest"""
    ...
    def setUp(self):
        ...
        self.sub_class_name = T414_UserDynamic_02NormalTest.__name__
        self.testure_path = r"testure/T414_UserDynamic_02NormalTest.csv"
        self.config_path = r'''../config.properties'''
        self.logger = logging.getLogger(self.__class__.__name__)
        self.initConfig()
        self.regist_func(self.testT414_UserDynamic_02Normal)
        ################################################################################

    def testT414_UserDynamic_02Normal(self):
        """
            测试用例主函数，若框架不能满足需求，可以重写相关beforeHttpTest, afterHttpTest, check, initConifg, checkJson
            等父类方法， 如:
            def beforeHttpTest(self, context):
                    super(T414_UserDynamic_02NormalTest, self).beforeHttpTest(context=context)
